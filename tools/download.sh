...
function usage() {
  ...
  echo
  echo "Copies Abydos data to proper locations from local copy of repository."
  echo "Assumes script is in repo tools directory."
  echo
  echo "Clone the repo:"
  printf '\t%s\n' 'git clone git@github.com:<owner>/abydos-data.git'
  echo
  ...
  echo
  echo set ABYDOS_DATA_DIR to target directory if different than /usr/share, e.g.:
  printf '\t%s %s\n' 'ABYDOS_DATA_DIR=./local/dir' "$(basename $0) book"
  echo
}
...
collection=$1
data_dir=${ABYDOS_DATA_DIR:-/usr/share/abydos-data}
